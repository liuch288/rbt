...
**核心逻辑**：
1. **买单成交判定**：卖一价 ≤ 买单价格 → 立即成交，或平均成交价 < 买单价格 → 成交
2. **卖单成交判定**：买一价 ≥ 卖单价格 → 立即成交，或平均成交价 > 卖单价格 → 成交
...
2. **订单簿分析**：分析盘口前5档价格和挂单量，计算同价位排单量（`volume_before_this_order`）
...
4. **阶段控制**：
   - `order_maintaining_time`: 挂单维持时间，超时后等待 spread 收窄以对手价平仓
...
2. **原价平仓阶段**：根据盈亏计算平仓价格
3. **对手价平仓阶段** (`active_closing_time`)：以对手价积极平仓
