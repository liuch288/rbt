...
class BtsSimplePEU(PnlEstimateUnit):
    ...
    def get_param_str(self):
        return f"{self.watching_time}_{self.buy_shift}_{self.sell_shift}"
    ...
    def estimate(self, future_data, *args, **kwargs) -> dict:
        ...
        buy_executable = md[
            (md["ask_px1"] <= buy_px)
            | ((md["exec_px"] + self.tick_size < buy_px) & (md["trade_sz"] > 0))
        ]
        buy_exec_time = None
        ...
        if buy_exec_time is not None:
            ...
            sell_executable = md[
                (md["bid_px1"] >= sell_px)
                | ((md["exec_px"] - self.tick_size > sell_px) & (md["trade_sz"] > 0))
            ]
